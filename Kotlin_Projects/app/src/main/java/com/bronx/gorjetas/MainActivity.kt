...
class MainActivity : AppCompatActivity() {
    ...
    fun valores(valueAux: String){
        ...
        this.tip15 = value + 0.15*value
        tip15pctTxt.text = "$$tip15"


            if(CustomValue == 0){
                this.tipCustom = (this.CustomValue.toDouble()/100.00)*value
                this.totalTipCustom = tipCustom
                tipCustompctTxt.text = "$$tipCustom"
                totalCustompctTxt.text = "$$totalTipCustom"
            } else {
                this.tipCustom = value + (this.CustomValue.toDouble()/100.00)*value
                tipCustompctTxt.text = "$$tipCustom"
                this.totalTipCustom = tipCustom + value
                totalCustompctTxt.text = "$$totalTipCustom"
            }

            this.totalTip15 = tip15 + value
